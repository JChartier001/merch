...
describe('server.js', () => {
    describe('index route', () => {
        ...
        it('should return a JSON object from the index route', async () => {
            const expectedBody = { status: "The Merch Dropper server is running!!" };
...
describe('User Router', () => {
    describe('CRUD Functionality', () => {
        it('Gets all users',  done => {
            request(server).get('/api/users/').then((response) => {
        ...
        it('Gets user by id but receives 404 error', done => {
            request(server).get('/api/users/1').then((response) => {
        ...
        it('Creates a user w/ username sam and deletes it', async () => {
            ...
            request(server).delete('/api/users/sam').then((response) => {
                expect(response.statusCode).toBe(200);
            })
        });
